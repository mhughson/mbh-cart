...
const unsigned char palette[16]={ 0x0f,0x0f,0x13,0x36,0x0f,0x17,0x29,0x20,0x0f,0x07,0x17,0x10,0x0f,0x2d,0x19,0x25 };
const unsigned char palette_bg[16]={ 0x0f,0x07,0x17,0x10,0x0f,0x03,0x11,0x30,0x0f,0x07,0x16,0x36,0x0f,0x2d,0x19,0x24 };
const unsigned char palette_title_bg[16]={ 0x0f,0x17,0x00,0x10,0x0f,0x16,0x00,0x36,0x0f,0x19,0x17,0x38,0x0f,0x17,0x13,0x36 };
...
game_actor particles[NUM_PARTICLES];
unsigned char cur_particle;
unsigned int temp16;
...
void go_to_state(unsigned char next_state)
{
	...
	switch (cur_state)
	{
		...
		case STATE_GAMEPLAY:
		{
			...
			memfill(particles, 0, NUM_PARTICLES * sizeof(game_actor));
			...
			do
			{
				...
				switch (loaded_obj_id - TYPE_ID_START_INDEX_TILED)
				{
					...
					case TYPE_BOULDER_SPAWNER:
					{
						...
						objs[y].vel_y = 0;
						objs[y].is_dead = 0;
						objs[y].type = TYPE_BOULDER_SPAWNER;
						++y;	
						break;
					}
...
#define GAME_OVER_ZERO_TILE (0x76)
